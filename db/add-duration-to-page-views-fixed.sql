...
SELECT 
  ...
  ROUND(
    (COUNT(CASE WHEN is_bounce = true THEN 1 END)::numeric / COUNT(*)::numeric) * 100, 
    2
  ) as bounce_rate_percentage,
  COUNT(CASE WHEN duration_seconds >= 30 THEN 1 END) as engaged_sessions,
  ROUND(
    (COUNT(CASE WHEN duration_seconds >= 30 THEN 1 END)::numeric / COUNT(*)::numeric) * 100, 
    2
...
SELECT 
  ...
  ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY duration_seconds::numeric), 2) as median_duration,
...
-- Policy to allow authenticated users to read page views
CREATE POLICY IF NOT EXISTS "Allow authenticated users to read page views" ON page_views
FOR SELECT TO authenticated USING (true);

-- Policy to allow anonymous users to insert page views
CREATE POLICY IF NOT EXISTS "Allow anonymous users to insert page views" ON page_views
FOR INSERT TO anon WITH CHECK (true);

-- Policy to allow authenticated users to update page views (for duration updates)
CREATE POLICY IF NOT EXISTS "Allow authenticated users to update page views" ON page_views
